...
pub fn process(input: &str) -> miette::Result<String> {
    ...
    // dbg!(&valid_manuals);
    let sorted_munuals: Vec<_> = valid_manuals.iter().map(|&manual| {
        let mut manual = manual.clone();
        manual.sort_by(|a, b| if page_order.get(a).unwrap_or(&vec![]).contains(b) {
    ...
    let middles_sum: i32 = sorted_munuals.iter().map(|manual| manual[manual.len() / 2]).sum();
